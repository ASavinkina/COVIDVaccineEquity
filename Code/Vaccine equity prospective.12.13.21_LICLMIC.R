...
Income <-World_bank_categories[(which(World_bank_categories$GroupCode %in% c("LMC","LIC"))),]

Income <- rename(Income,  "Income"="GroupName",
                 "IncomeCode"="GroupCode")

Region2 <- merge(x=Income, y=Region, by=("iso_code"), all.x=TRUE)

#Region2 <- Region2[,c("CountryCode.x","CountryName.x", "Income","Region")]

#write.csv(Region2, file="~/Documents/Rotations/Gonsalves/Results/Regions_low_income.csv", row.names = FALSE)
...
#write.csv(Vaccine_equity_region, file="~/Documents/Rotations/Gonsalves/Results/Vaccine_equity_for_SA_12.13.21_lowincome.csv", row.names = FALSE)
...
IFR = 5/1000
vaccine_mortality_efficacy= 0.5
cost_of_dose = 10
...
Vaccine_equity_region <- Vaccine_equity_region %>%
  mutate(vaccine_needed = (vaccinated_percent - fully_vaccinated_percent)*region_pop + 2* (1-vaccinated_percent)*region_pop)

write.csv(Vaccine_equity_region, file="Results/Vaccine_equity_for_SA_12.13.21_lowincome.csv", row.names = FALSE)
...
write.csv(Vaccine_equity_region_final1, file="Results/Vaccine_equity_allregions_12.13.21_lowincome.csv", row.names = FALSE)
