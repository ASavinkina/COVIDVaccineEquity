---
title: "Vaccine Equity Globally LIC and LMIC, two dose"
...
deaths_averted <- function(params){
  ...
  Vaccine_equity_region <- Vaccine_equity_region %>%
    mutate(vaccine_needed = (vaccinated_percent - fully_vaccinated_percent)*region_pop + 2*
             (1-vaccinated_percent)*region_pop)
...
# Calculate number at risk, deaths averted, and vaccination percent
 ...
 mutate(number_at_risk_covid_deaths2 = ((1-infection_percent_covid_deaths)*(1-fully_vaccinated_percent)+
           infection_percent_covid_deaths * (1-fully_vaccinated_percent) * (1-natural_immunity)) *region_pop,
         
         number_at_risk_excess_deaths2 = ((1-infection_percent_excess_deaths)*(1-fully_vaccinated_percent)+
           infection_percent_excess_deaths * (1-fully_vaccinated_percent) * (1-natural_immunity)) *region_pop,
  
          number_at_risk_excess_deaths_lower52 = ((1-infection_percent_excess_deaths_lower5)*(1-fully_vaccinated_percent)+
           infection_percent_excess_deaths_lower5 * (1-fully_vaccinated_percent) * (1-natural_immunity)) *region_pop,
        
              number_at_risk_excess_deaths_upper52 = ((1-infection_percent_excess_deaths_upper5)*(1-fully_vaccinated_percent)+
           infection_percent_excess_deaths_upper5 * (1-fully_vaccinated_percent) * (1-natural_immunity)) *region_pop,
...
low_low_data <- 
  list(
    IFR_vaccinated = rep(3.3/100000,each=6),
    IFR_old = rep(5/1000,each=6),
    IFR_new = rep(0.5/1000,each=6),
    vaccine_mortality_efficacy= rep(0.8,each=6),
...
high_high_data <- 
  list(
    ...
    IFR_old = rep(5/1000,each=6),
    IFR_new = rep(5/1000,each=6),
    vaccine_mortality_efficacy= rep(0.95,each=6),
    cost_of_dose = seq(from=5, to=10, by=1),
    natural_immunity = rep(0.8,each=6)
  )
...
low_hesitant_data <- 
  list(
    ...
    IFR_old = rep(5/1000,each=6),
    IFR_new = rep(.5/1000,each=6),
    vaccine_mortality_efficacy= rep(0.6,each=6),
    cost_of_dose = seq(from=5, to=10, by=1),
    natural_immunity = rep(0.8,each=6)
  )
...
vaccine_efficacy_high <- "95%"
vaccine_efficacy_low <- "80%"
vaccine_efficacy_hesitancy <- "60%"
...
Deaths$vaccine_efficacy <- c("80%","80%","80%","95%","95%","95%","70%","70%","70%")
...
Deaths$vaccine_efficacy <- factor(Deaths$vaccine_efficacy, levels=c("70%","80%","95%"))
...
### Assuming "pre-omicron" IFR= 5/1000, omicron IFR= 1/1000, vaccine efficacy against mortality = 80%, natural immunity efficacy against mortality of 80%, and cost per dose= $7, 
...
basecase <- 
  list(
    IFR_old = 5/1000,
    IFR_new = 1/1000,
    vaccine_mortality_efficacy= 0.8,
    cost_of_dose = 7,
...
basecase <- 
  list(
    IFR_old = 5/1000,
    IFR_new = 1/1000,
    vaccine_mortality_efficacy= 0.6,
    cost_of_dose = 7,
    natural_immunity=0.8
  )
...
write.csv(cost_per_death_excess, file="~/Documents/Rotations/Gonsalves/Code/Rmarkdown/For_graph_cost_twodose.csv")
...
write.csv(cost_per_death_hesitant, file="~/Documents/Rotations/Gonsalves/Code/Rmarkdown/For_graph_cost_twodose_hesitant.csv")

write.csv(Deaths, file="~/Documents/Rotations/Gonsalves/Code/Rmarkdown/For_graph_deaths_twodose.csv")
