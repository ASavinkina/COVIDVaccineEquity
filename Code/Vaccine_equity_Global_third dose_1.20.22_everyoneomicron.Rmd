...
Vaccine_equity_region <- read.csv(file="~/Documents/Rotations/Gonsalves/COVIDVaccineEquity/Results/Vaccine_equity_for_SA_12.13.21_lowincome.csv")

# Limit data to regions of interest
...
deaths_averted <- function(params){
  ...
  # 
    ...
    #For this sensitivity analysis- we assume 100% of people have been infected (will have prior immunity)
  
 infection_percent_excess_deaths <- 1 
 infection_percent_covid_deaths <- 1
 infection_percent_excess_deaths_lower5 <- 1
 infection_percent_excess_deaths_upper5 <- 1
...
low_hesitant_data <- 
  list(
    IFR_old = rep(5/1000,each=6),
    IFR_new = rep(.5/1000,each=6),
    vaccine_mortality_efficacy= rep(0.75,each=6),
    vaccine_mortality_efficacy_two = rep(0.7, each=6),
    cost_of_dose = seq(from=5, to=10, by=1),
    natural_immunity = rep(0.8,each=6)
  )
